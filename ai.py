...
【输出严格 JSON 格式】（不要 markdown 包裹，不要任何解释，直接返回 JSON）：
{
  ...
  "dimensions": {
    "visual_impact": {
      "score": <int 0-100>,
      "diagnosis": "<25 字以内，该维度的**缺点 / 问题**（不写亮点和好的地方，只指出哪里不行、为什么不行），用专业术语>",
  ...
  "improvement_prompt": "<给 gpt-image edit API 用的英文 prompt，80-130 词（控制短而聚焦）。**只改 1-2 个**得分最低的维度，不要全面重做。指令保守：「lightly refine / slightly adjust / mildly enhance」。明确说「Keep about 85% of the original visual unchanged」。**禁止**写「BOLDLY / DRAMATICALLY / replace background / restyle everything」这种放飞词。>"
}

【⭐ diagnosis 字段硬规则 — 绝对禁止写优点】
绝大多数得分 70+ 的维度，AI 倾向写正面评价，**这是错误行为**。强制规则：

- diagnosis 字段 **只能写缺点 / 问题 / 不足**。
- **禁词列表**（写了一律重写）：合理、统一、突出、协调、良好、不错、足够、清晰、和谐、平衡、得当、到位、明显、吸引、好的、佳、强、丰富、亮眼。
- 即使该维度得分 90+，也要找出**最小的瑕疵**作为 diagnosis 写出来（哪怕是"再 X 一点会更好"也好过夸）。
- 反面教材：「色彩鲜明，吸引眼球」← 这是优点，违规
- 反面教材：「构图合理，留白适中」← 优点，违规
- 反面教材：「色调统一，情绪传达良好」← 优点，违规
- 正确示范（得分 85 的色彩维度）：「饱和度偏高，缺乏冷暖呼吸节奏」
- 正确示范（得分 80 的构图维度）：「主体偏左侧，右半场视觉权重过低」
- 正确示范（得分 75 的文字维度）：「副标题对比度不足，3 米外几乎不可读」

【⭐ improvement_prompt 字段硬规则 — 改动节制】
gpt-image edit 默认会按 prompt 列举的维度全部改一遍。**列得越多改得越乱**。约束：

- prompt 必须**聚焦 1-2 个**最严重的问题，**绝对不能列 3 个以上改动维度**。
- 必须明确说 "Keep about 85% of the original visual unchanged, only改 X"。
- 用 "lightly / slightly / mildly" 这种温和动词，禁用 "BOLDLY / DRAMATICALLY / completely / replace"。
- 长度控制 80-130 词（短而具体）。
- 反面：「Refine title, adjust colors, add border, restyle background...」← 列了 4 个，违规
- 正确（聚焦字体）：「Keep about 85% of the original cover unchanged including subject pose, background, and color palette. Only lightly refine the title typography: slightly increase font weight, adjust position to upper-left for better balance. Do not change the background or color scheme.」
