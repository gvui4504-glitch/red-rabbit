...
【输出严格 JSON 格式】（不要 markdown 包裹，不要任何解释，直接返回 JSON）：
{
  ...
  "dimensions": {
    "visual_impact": {
      "score": <int 0-100>,
      "diagnosis": "<25 字以内，核心病灶或亮点，用专业术语>",
  ...
  "improvement_prompt": "<给 gpt-image 的 edit API 用的英文 prompt，130-200 词。**适度改进**：保留原图主体（人物 / 产品的身份、姿态、大致位置），但其他要素要有**清晰可见的改动**——挑 2-3 个维度（背景、标题字体、色调、装饰元素）明确改。**禁用**「BOLDLY / DRAMATICALLY / AGGRESSIVELY / COMPLETELY / totally / replace EVERYTHING」这种激进词（AI 会改飞主体）；**也禁用**「subtle / slightly / minor / refine / adjust」这种保守词（看不出改动）。**用**「clearly redesign / notably restyle / change X to Y / replace background with」这种中性方向词。>"
}
...
【⭐ improvement_prompt 的硬性写作规则】（v1.5 平衡版）
gpt-image edit 默认偏保守。prompt 要明确指示改动，让改进版**跟原图有清晰可辨的不同**，但**不要过激导致主体走样**。

模板：
```
Keep the main subject (identity, pose, rough position).
Clearly improve 2-3 of these aspects:
- Background: replace with <a specific style/mood, e.g., clean pastel pink gradient>
- Title typography: <notably larger/bolder font, new placement, contrast color>
- Color palette: <change overall mood from current to e.g., warmer + softer>
- Visual elements: <add a small badge / decorative border / subtle texture>
The result should be visibly improved while the main subject is still clearly recognizable.
```

❌ 反例（v1 太保守）：「Refine title slightly, adjust colors a bit.」 → 看不出区别
❌ 反例（v2 太放飞）：「BOLDLY redesign EVERYTHING. DRAMATICALLY transform background AND title AND colors.」 → 主体也变了
✅ 正例（v1.5 平衡）：「Keep subject's pose. Replace cluttered background with clean pastel-pink gradient. Restyle title in bolder sans-serif, 1.5x larger, top-left, dark navy on cream tag. Shift overall palette slightly warmer with peach accent.」"""
