...
【输出严格 JSON 格式】（不要 markdown 包裹，不要任何解释，直接返回 JSON）：
{
  ...
  "improvement_prompt": "<给 gpt-image 的 edit API 用的英文 prompt，100-150 词。目标：改进图跟原图有约 30-40% 的视觉变化（明显但不离谱）。**必须保留**：主体身份、姿态、面部特征、大致位置（能认出是同一个人/物）。**可以改**：背景 / 标题字号字体颜色位置 / 整体色调 / 装饰元素 — 选 1-3 项搭配调整。用「noticeably / visibly improve / modernize」这种中性偏强的指令词。>"
}
...
【improvement_prompt 写作要点】
- 主体（人物/产品）必须能认出 → 用 "Keep the main subject's identity, pose, and position"
- 但其他元素（背景 / 文字 / 装饰 / 色调）要有**明显**的改变（用户一眼能看出"啊改了")
- 禁用：「BOLDLY / DRAMATICALLY / completely / replace everything」← 改太离谱
- 禁用：「slightly / subtly / refine / minimally」← 改不出来
- 推荐：「noticeably enlarge / visibly improve / modernize / clearly redesign the background / refresh the color palette」
- 结尾加一句「Result should be about 30-40% visually different from the original — clearly improved but the subject is recognizably the same.」
- 示例：「Keep the main subject's identity, pose, and position. Modernize the cover: replace the plain background with a soft pastel pink gradient, noticeably enlarge the main title with a bolder sans-serif font in dark navy, add a small decorative star icon in the top-right corner. The overall mood shifts to a warmer, more inviting feel. Result should be about 30-40% visually different from the original — clearly improved but the subject is recognizably the same.」"""
