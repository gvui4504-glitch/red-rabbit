...
【输出严格 JSON 格式】（不要 markdown 包裹，不要任何解释，直接返回 JSON）：
{
  ...
  "improvement_prompt": "<给 gpt-image edit API 用的英文 prompt，150-220 词。**激进改写**：除了原图主体的身份/姿态保留，**其他要素全部敢于重做**——背景重画、标题字体彻底重排、色调换情绪、加新视觉元素。目标是让改进版跟原图**一眼就能看出明显不同**。**禁用**「refine / slightly / subtle / minor」这种保守词；**多用**「BOLDLY redesign / DRAMATICALLY transform / COMPLETELY restyle / AGGRESSIVELY change」。**至少同时改 3 个维度**（背景 + 标题 + 色调 + 装饰）。>"
}
...
【⭐ improvement_prompt 的硬性写作规则】（v2 强化）
gpt-image edit 默认很保守，prompt 必须"激进"才能让改进版与原图视觉**明显不同**。模板：
```
Keep the original main subject's identity, pose, and rough position.
BOLDLY redesign EVERYTHING else:
- Background: <completely replace with X style / mood>
- Title typography: <dramatically restyle font weight, size 2x, new position, bold color contrast>
- Color palette: <shift from current mood to NEW mood, e.g., warm pastel → high-contrast neon>
- Add visual elements: <new icons / borders / texture / lighting accents>
The result MUST be visibly different from the original at first glance.
Do NOT preserve the original background, typography, or color scheme.
```
❌ 反面教材："Keep main subject. Refine title style and slightly adjust colors."
✅ 正面教材："Keep the subject's pose. BOLDLY replace cluttered background with clean pastel-pink gradient. DRAMATICALLY restyle title: bold sans-serif, 2.5x larger, top-left corner, dark navy on cream sticker badge. Shift overall palette from cool blue to warm peach + mint accent. Add subtle dotted border and a small decorative star icon top-right." """
