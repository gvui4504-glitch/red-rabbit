...
def consume_image_quota() -> None:
    """显式消耗 1 次生图配额。

    新设计下生图按"次"计：1 次按钮 = 1 次 quota = 内部产出多张图。
    UI 层在生图开始前调用此函数预扣配额，generate_improved_cover() 自身不再扣配额。
    没配额 → 抛 NoQuotaError。
    """
    _consume_image()
...
【输出严格 JSON 格式】（不要 markdown 包裹，不要任何解释，直接返回 JSON）：
{
  ...
  "improvement_prompts": [
    {
      "angle": "<改进方向标签，4-6 个汉字，比如「标题强化」「构图重构」「色彩调优」「视觉重心」>",
      "prompt": "<给 gpt-image 的 edit API 用的英文 prompt，120-180 词。这是图生图 API，会保留原图主体。prompt 应该是「Keep main subject. Refine XXX」的指令式语气，而不是从零生成的描述。**不要重新描述主体**，因为主体会保留。>"
    },
    {
      "angle": "<必须跟第一个 angle 显著不同的方向>",
      "prompt": "<同上格式>"
    },
    {
      "angle": "<再一个显著不同的方向>",
...
【⭐ 关于 improvement_prompts 的硬性要求】
- **必须 3 个**，不能少
- **3 个 angle 必须显著不同方向**，不是同一方向的 3 个变体。例如：
  - ✅ 方向 1「标题强化」改字体/字号/对比；方向 2「构图重构」改主体位置/留白；方向 3「色彩调优」改色调/饱和度 — 3 个完全不同维度
  - ❌ 方向 1「字号放大」；方向 2「字体加粗」；方向 3「字色变红」— 都是字体相关，太相似
- **每个 prompt 都用英文写**，因为 image edit 模型对英文 prompt 敏感度更高
- 每个 prompt 都以 "Keep the main subject of the original cover." 开头，确保主体保留

【风格要求】
- 中文输出（除 prompt 字段必须英文）
- 用行话但让用户能懂（别纯术语堆砌）
...
def generate_improved_cover(
    ...
    """基于原图 + prompt 生成 1 张改进版封面（图生图 / edit 模式，保留原图主体）。

    注意：此函数**不消耗 quota**。配额由调用方（UI 层）显式 _consume_image() 控制。
    设计理由：一次"生图操作"消耗 1 次 quota 但产出 3 张图，所以 quota 解耦到外层。
