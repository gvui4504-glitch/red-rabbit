...
【输出严格 JSON 格式】（不要 markdown 包裹，不要任何解释，直接返回 JSON）：
{
  ...
  "improvement_prompt": "<给 gpt-image 的 edit API 用的英文 prompt，120-180 词。这是图生图 API，会保留原图主体，所以 prompt 应该是「改这张图的 XXX」的指令式语气，而不是「从零生成一张 XXX」的描述。重点说明：要改动的具体元素（比如调色、加文字、改字号、加边框、重排布局）+ 风格方向。**不要重新描述主体**，因为主体会保留。>"
}
...
- 允许指出严重问题，语气专业不刻薄
- improvement_prompt 是给图生图模型的 edit 指令：「Keep the main subject. Refine: bigger title typography in upper-left, increase color contrast, add a soft pastel background...」这种风格"""
